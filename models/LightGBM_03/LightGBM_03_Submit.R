...
#     data <- (.)
# 
#     df.morphes <- get_morphological_data(NULL, "models/LightGBM_03/data/input/morphological_data.csv")
# 
#     data %>%
# 
#       # 行・文字単位の特徴量を追加
...
# 
#       # 句点・読点の直前の品詞情報を追加
#       add_pos_ratio_before_period(df.morphes, "。", "reading_point_") %>%
#       add_pos_ratio_before_period(df.morphes, "、", "period_")        %>%
# 
#       # 形態素関連の特徴量を追加
#       add_morph_info(df.morphes) %>%
# 
#       # ファイルへの書き出し
#       write_test_data("models/LightGBM_03/data/input/test_data.csv")
# }
...
lst.train_valid_test <- rec %>%
  {
    ...
    # train data
    df.train <- read_train_data("models/LightGBM_03/data/input/train_data.csv") %>%
      # recipe の適用
    ...
    x.test <- read_test_data("models/LightGBM_03/data/input/test_data.csv") %>%
      # recipe の適用
      recipes::bake(object = recipe, new_data = .) %>%
...
df.grid.params <- tibble(
  ...
  tidyr::crossing(
    seed = sample(1:10000, size = 5, replace = F)
#    seed = 1234
  )
...
system.time({
  ...
  furrr::future_pmap_dfr(df.grid.params, function(
    ...
    # ファイルに出力
    {
      ...
      # 出力ファイルパス
      filepath <- stringr::str_c("models/LightGBM_03/data/output", filename, sep = "/")
